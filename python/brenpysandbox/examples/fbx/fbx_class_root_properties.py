"""Create new default properties for all objects of the same type.
...
TEST_EXPORT_FILE = os.path.join(
    DUMP_DIR,
    "fbx_class_root_properties_example_01.fbx"
)


def create_scene(fbx_manager):
    """Create a scene with some nodes, and some class root properties.
    """
    ...
    # get root property
    if True:
        root_prop = object_1.GetClassRootProperty()
    else:
        # note that we can only get the class root property
        # after an object has been created
        # the following code will error
        root_prop = fbx.FbxObject.GetClassRootProperty()

    # note that the object root property is not
    # the same as the class root property
    print "root equal", root_prop == object_1.RootProperty
    print "RootProperty", object_1.RootProperty.GetName()

    # note that root property is valid at this point
    print root_prop
    print "root valid", root_prop.IsValid()

    # create a reference property and connect nodes
    ref_prop = fbx.FbxProperty.Create(
        root_prop, fbx.FbxReferenceDT, "testReferenceProperty"
    )
    ...
    if False:
        # note that connecting a class root property
        # makes it invalid!
    ...
    ref_dbl2_prop = fbx.FbxProperty.Create(
        root_prop, fbx.FbxDouble2DT, "testReferenceDbl2Property"
    )
...
#     ref_dbl2_prop.ConnectSrcObject(node_1)
    ...
    # we can add child properties as normal
    child_prop_1 = fbx.FbxProperty.Create(
        ref_dbl2_prop, fbx.FbxDouble2DT, "child_property1"
    )

    child_prop_1.Set(19.0)
    ...
    ref_dbl3_prop = fbx.FbxProperty.Create(
        root_prop, fbx.FbxDouble2DT, "testReferenceDbl2Property2"
    )
...
def print_child_properties(fbx_property, indent, skip=False):
...
def print_root_properties(fbx_object):
    ...
    print_child_properties(root_prop, 1, skip=True)
